...
def init_exp():
	...
	exp = {'pro': langData, 'per': courseData}
...
def login_user():
	...
	login = account["login"]
	hash_pwd = login["pwd"]
	if check_pwd(input_pwd,hash_pwd):
...
def signup_errors(first, last, s_id, email, pwd, confirm):
	...
	then assigns any valid values to the corresponding
	...
	if not pwd:
		...
		flask.session['pwd'] = encrypt_pwd(pwd)
